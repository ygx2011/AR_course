...
int ORBmatcher::SearchByProjectionWithOptimizationAlan(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    int nmatches = 0;
    ...
    int nLastOctave;
    cv::Mat dMP;
    int bestDist, bestIdx2;
    vector<size_t>::const_iterator vit, vend;
    size_t i2;
    float ur, er;
    int dist;
    vector<size_t> vIndices2;
    ...
    for(int i=0; i<LastFrame.N; ++i)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                for(vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
                {
                    i2 = *vit;
                    if(CurrentFrame.mvpMapPoints[i2])
                        if(CurrentFrame.mvpMapPoints[i2]->Observations()>0)
                    ...
                    if(CurrentFrame.mvuRight[i2]>0)
                    {
                        ur = u - CurrentFrame.mbf*invzc;
                        er = fabs(ur - CurrentFrame.mvuRight[i2]);
                        if(er>radius)
                            continue;
                    }
