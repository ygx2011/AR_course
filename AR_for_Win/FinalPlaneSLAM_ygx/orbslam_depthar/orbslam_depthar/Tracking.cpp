...
Tracking::Tracking(ORBVocabulary* pVoc, 
	...
	Map *pMap, string strSettingPath/*,cv::VideoCapture& capCam*/) :
    mState(NO_IMAGES_YET), mpORBVocabulary(pVoc),
		...
		mbReseting(false), mbForceRelocalisation(false), 
		mbMotionModel(false)//, mCamera(capCam)
{
...
{
	cv::Mat rVec;
    cv::Rodrigues(pose_R, rVec);
    cv::Mat tVec = pose_T.clone();
...
//////////////////////////////////////////////////////////////////////////////
std::vector<cv::Point3f> Tracking::Get_allMapPoints()
{  
    const vector<MapPoint*> vpMPs = mpMap->GetAllMapPoints();
...
void Tracking::FirstInitialization()
{
    ...
    if(mCurrentFrame.mvKeys.size()>50)
    {
		...
		cout<<"YGX---!mpInitializer: "<<frame_count<<endl;
...
void Tracking::Initialize()
{
    ...
    if(mCurrentFrame.mvKeys.size()<=50)
    {
		...
		cout<<"YGX---Check if current frame has enough keypoints: "<<frame_count<<endl;
    ...
    if(nmatches<50)
    {
		...
		cout<<"YGX---Check if there are enough correspondences: "<<frame_count<<endl;
