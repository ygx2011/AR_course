...
//  markerlessar
//
//  Created by Gaoxuan Ying on 2021/12/1.
//
...
extern "C" __declspec(dllexport) ERROR_CODE process_Image(uchar [],float [],float [],bool &);
...
ERROR_CODE Initialize(int deviceID)
{
	...
	capCam.set(CAP_PROP_FRAME_WIDTH, 640);
	capCam.set(CAP_PROP_FRAME_HEIGHT, 480);
	capCam.set(CAP_PROP_FRAME_COUNT, 30);

	template_img = imread("C:/Users/ethan/Desktop/AR_for_MacOS/markerlessAR/x64/Release/test.png");
...
ERROR_CODE process_Image(uchar imgData[], float R[], float T[], bool &isShow)
{
	Mat img;
	capCam >> img;

	Mat im_ygx = img.clone();
	cvtColor(im_ygx, im_ygx, COLOR_BGR2RGB);
		...
		if (istracking == false)
		{
			...
			if (keyPoints_2.size() >= 10)
			{
				...
				if (train_p.size() >= 4 && train_p.size() >= (int)(0.1*descriptors_1.rows))
				{
					...
					cv::line(img, scene_corners[0], scene_corners[1], cv::Scalar(255, 0, 0, 255), 2);
					cv::line(img, scene_corners[1], scene_corners[2], cv::Scalar(0, 255, 0, 255), 2);
					cv::line(img, scene_corners[2], scene_corners[3], cv::Scalar(0, 0, 255, 255), 2);
					cv::line(img, scene_corners[3], scene_corners[0], cv::Scalar(0, 0, 0, 255), 2);
			...
			if (prev_keyPoints.size() >= 15)
			{
				...
				if (tr_num >= 15)
				{
					...
					if (countNonZero(homographyMat) != 0)
					{
						...
						{
							cv::line(img, prev_corners[0], prev_corners[1], cv::Scalar(255, 0, 0, 255), 2);
							cv::line(img, prev_corners[1], prev_corners[2], cv::Scalar(0, 255, 0, 255), 2);
							cv::line(img, prev_corners[2], prev_corners[3], cv::Scalar(0, 0, 255, 255), 2);
							cv::line(img, prev_corners[3], prev_corners[0], cv::Scalar(0, 0, 0, 255), 2);
...
int main()
{
	...
	while (true)
	{
		process_Image(imgData, R, T, isshow);
		cnt++;
	}
	Release();
	return 0;
