...
int main( void )
{
    ...
    ORB_SLAM2::System SLAM("../../ORB_Vocabulary/ORBvoc.bin", "../../MonocularCam.yaml", ORB_SLAM2::System::MONOCULAR,false);
    ...
    window = glfwCreateWindow( 640, 480, "Monocular-ORB-SLAM-AR", NULL, NULL);
    if( window == NULL ){
    ...
    do{
        ...
        else
        {
            ...
            {
                char save_name[100];
                sprintf(save_name, "%s%d%s", "../../init/img", frame_count, ".png");
                Mat img = imread(save_name);
                
                orb_tracker.process(img, false);
                
                CameraPose = SLAM.TrackMonocular(img, 0);
            }
            
            if(frame_count > 217)
            {
                CameraPose = SLAM.TrackMonocular(frame_left, 0);
            }
            
            //frame_count++;
            //CameraPose = SLAM.TrackMonocular(frame_left, 0);
            //char save_name[100];
            //sprintf(save_name, "%s%d%s", "img", frame_count, ".png");
            //cv::imwrite(save_name, frame_left);
