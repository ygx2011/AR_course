...
bool Tracker::process(const Mat frame_left, bool slamMode)
{
    ...
    for(unsigned i = 0; i < matches.size(); i++)
    {
        ...
        {
            query_p.push_back(first_kp[matches[i][0].queryIdx].pt);
            train_p.push_back(      kp[matches[i][0].trainIdx].pt);
        }
    }
    ...
    if(train_p.size() >= thd)
    {
        homography = findHomography( query_p, train_p, RANSAC, 10.0f, inlier_mask);
    }
